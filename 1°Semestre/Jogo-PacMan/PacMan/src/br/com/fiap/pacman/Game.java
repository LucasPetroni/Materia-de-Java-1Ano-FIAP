...
public class Game extends JFrame implements KeyListener {
	...
	private Ghost ghost1 = new Ghost(20,20,0);
	...
	private Booster booster = new Booster(100, 150, 50);
	...
	private int turnosDeInvencibilidade = 0;
	...
	private void render() {
		...
		setTitle("Life: " + player.getLife() + " " + turnosDeInvencibilidade);
	...
	private void run() {
		while (player.getLife() > 0) {
			if (player.isInvincible()) turnosDeInvencibilidade--;
			if (turnosDeInvencibilidade == 0) player.setInvincible(false);
			...
			if (player.colide(booster)) {
				...
				turnosDeInvencibilidade = booster.getDuration();
